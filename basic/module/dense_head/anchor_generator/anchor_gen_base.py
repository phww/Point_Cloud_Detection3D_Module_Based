...
class AnchorGenerator:
    ...
    def __init__(self, anchor_gen_cfg, model_info_dict, class_type='Car',
        ...
        if self.stride is None:  # if don't set stride in cfg,auto set stride
            ...
            shape = np.array(self.feature_map_size, np.float32)[[2, 1, 0]]  # W H D
    ...
    def _gen_anchor_stride(self):
        ...
        # D, H, W == Z Y X
        z_centers = torch.arange(self.feature_map_size[0], dtype=self.dtype)  # [0, D]
        y_centers = torch.arange(self.feature_map_size[1], dtype=self.dtype)  # [0, H]
        x_centers = torch.arange(self.feature_map_size[2], dtype=self.dtype)  # [0, W]
