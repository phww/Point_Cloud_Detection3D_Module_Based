...
class MaxIouTargetAssigner(AssignerBase):
    ...
    def make_batch_gt_bbox_tuples(self, ious, pos_thr, neg_thr, batch_id):
        ...
        if self.force_match:
            ...
            pos_gt_ids = torch.arange(ious.size(0)) + 1
            pos_batch_ids = torch.ones_like(argmax_gt) * batch_id
            pos_tuples_0 = torch.stack([pos_batch_ids.cpu(), pos_gt_ids.cpu(), argmax_gt.cpu()], dim=1).to(self.device)
        ...
        if pos_tuples_0 is not None:
            pos_tuples = torch.cat([pos_tuples_0, pos_tuples], dim=0)
