...
def put_data_to_gpu(data_dict):
    for key, value in data_dict.items():
        if not isinstance(value, np.ndarray):
            continue
        elif key in ['frame_id', 'pts_coord', 'calib']:
            continue
        elif key in ['image_shape']:
            data_dict[key] = torch.from_numpy(value).int().cuda()
        else:
            data_dict[key] = torch.from_numpy(value).float().cuda()
